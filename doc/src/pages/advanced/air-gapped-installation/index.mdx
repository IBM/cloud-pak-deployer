...
* Optionally, the bastion server must connect to the internet via a proxy server, see [Running behind a proxy](#running-behind-a-proxy)
...
```
* **Diagram step 3**. Ship the following directories from the internet-connected server to the bastion server:
...
* The external internet-connected server **cannot** connect to the private cloud
...
./cp-deploy.sh env save
```
* **Diagram step 3**. Ship the `cloud-pak-deployer` direcory and configuration and status directories to a server that can be connected to the private registry and the OpenShift cluster. 

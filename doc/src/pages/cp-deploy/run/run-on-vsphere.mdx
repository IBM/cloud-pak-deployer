...
![vSphere configuration](sample-vsphere-nfs-ocs-cp4d.png)
...
```
./cp-deploy.sh env apply -e env_id=pluto-01 --check-only
```
...
You can find examples of a couple of typical changes you may want to do here: [Post-run changes](/cpd-deploy/post-run).

...
tabs: ['Run on IBM Cloud', 'Run on existing OpenShift']
---
...
./cp-deploy.sh vault set \
    --vault-secret sample-kubeconfig 
    --vault-secret-file ~/.kube/config
```

`sample-kubeconfig` is a composition of the cluster name you specified in the `openshift` configuration and the fixed string `-kubeconfig`. The deployer will retrieve this secret from the vault when it requires access to OpenShift. If the secret cannot be found or if it is invalid or the OpenShift login token has expired, the deployer will fail and you will need to update the secret with a valid Kubernetes config file.

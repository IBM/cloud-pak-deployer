...
IBM Cloud Pak for Data can connect to an LDAP user registry in order for users to connect with their LDAP credentials. The configuration of LDAP can be specified in a seperate yaml file in the `config` folder:  [Example LDAP and User Group settings](/cpd-design/objects/ldap_user_groups).

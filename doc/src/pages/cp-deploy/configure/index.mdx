...
```
2. Edit the "config/cp4d-....yaml" file and select the cartridges to be installed by changing the state to `installed`. Additionally you can accept the Cloud Pak license in the config file by specifying `accept_licenses: True`.
```
...
```
    ...
    cd /data/config/sample
```
2. Copy the relevant "ocp-..." file from the `./config-samples` direcotry to the `./config` directory, for example:
```
    cp ./config-samples/ocp-ibm-cloud-roks-ocs.yaml ./config/
```
3. Copy the relevant "cp4d-..." file from the `./config-samples` direcotry to the `./config` directory, for example:
```
    cp ./config-samples/cp4d-408.yaml ./config/
```
4. Edit the "config/cp4d-....yaml" file and select the cartridges to be installed by changing the state to `installed`. Additionally you can accept the Cloud Pak license in the config file by specifying `accept_licenses: True`.
```
    nano ./config/cp4d-408.yaml
```
...
The Cloud Pak Deployer pipeline has been built using Ansible and it can be configured using "inventory" files. Inventory files allow you to specify global variables used throughout Ansible playbooks. In the current version of the Cloud Pak Deployer, the inventory directory has become fully optional as the `global_config` and `vault` objects have taken over its role.

...
If you plan to provision a ROKS on VPC cluster, there is no need to change address prefixes and subnets. The IP addresses used by the provisioned components are private to the VPC and are not externally exposed.

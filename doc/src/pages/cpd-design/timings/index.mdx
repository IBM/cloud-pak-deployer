...
| | | | |
| 50 - Install Cloud Pak | Prepare OpenShift for Cloud Pak for Data install | 60 | During this step, the worker nodes may be replaced and also the Kubernetes services may be restarted. |
...
| | Create Cloud Pak for Data 4.0 subscriptions for cartridges | 15 | |
| | Install cartridges | 20-300 | The amount of time really depends on the cartridges being installed. In the table below you will find an estimate of the installation time for each cartridge. When installing Cloud Pak for Data 4.0, cartridges will be installed in parallel. |
| | | | |
...
## Cloud Pak for Data 4.0 cartridge deployment

| Cartridge | Full name  | Installation time | Instance provisioning time | Dependencies |
| --- | ----  | -----:| ---: | --- |
| lite | Cloud Pak for Data control plane | 20 | N/A | |
...
* Cloud Pak for Data installation with Cognos Analytics and Watson Studio will take 20 (control plane) + 75 (ccs) + 45 (wsl+ca) + 45 (ca instance) = ~185 minutes

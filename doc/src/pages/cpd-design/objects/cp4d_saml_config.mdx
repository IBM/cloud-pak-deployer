...
| -------- | -------------------------------------------------------------------- | --------- | -------------- |
| project  | Name of OpenShift project of the `cp4d` entry                        | Yes       |  |
...
The SAML configuration file in the Cloud Pak for Data user management pods holds one more field: `callbackUrl`. This field is automatically populated by the deployer and consists of the Cloud Pak for Data base URL appended with `/auth/login/sso/callback`.

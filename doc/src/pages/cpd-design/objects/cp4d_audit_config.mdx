...
## `cp4d_audit_config`
A `cp4d_audit_config` entry defines the audit configuration for a specific Cloud Pak for Data instance. The main configuration items are the number of replicas and the output. Currently only one output type is supported: `stdout`, which allows the OpenShift logging framework to pick up audit entries and forward to the designated collectors.
...
| audit_output.type | Type of auditing output, defines where audit logging entries will be written | Yes   | stdout |

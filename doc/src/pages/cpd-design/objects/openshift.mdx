...
| storage_classes.ocs_storage_size | Size of the OCS storage in Gibibytes (Gi) | Yes if `storage_type` is `ocs` |  |
...
| storage_classes.name | Name of the storage definition, to be referenced by the Cloud Pak | Yes |  |
| storage_classes.storage_type | Type of storage class to create in the OpenShift cluster | Yes | nfs |
| storage_classes.ocp_storage_class_file | OpenShift storage class to use for file storage, defaults to `managed-nfs-storage` | No | |
| storage_classes.ocp_storage_class_block | OpenShift storage class to use for block storage, defaults to `managed-nfs-storage` | No | |
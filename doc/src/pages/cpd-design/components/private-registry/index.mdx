...
## Using a private registry for the Cloud Pak installation
...
* Run the deployer with `vault set` subcommand and action and the `--air-gapped` flag to create the "kubeconfig" secret to connect to the OpenShift cluster
* Run the deployer with `env apply` subcommand and action and the `--air-gapped` flag to deploy the Cloud Pak
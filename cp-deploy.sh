...
command_usage() {
  ...
  echo "  --config-git-ref <ref>        Reference (branch, tag, commit ID) of the git repo. Will use default branch if not specified (\$CPD_CONFIG_GIT_REF)"
  echo "  --config-git-context <ctx>    Context (directory) within the git repo. Will use the root if not specified (\$CPD_CONFIG_GIT_CONTEXT)"

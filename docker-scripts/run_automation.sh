...
if [ -v GIT_CONFIG_REPO_URL ];then
    ...
    if [ ! -v CONTEXT_DIR ];then
        echo "Error: environment variable CONTEXT_DIR has not been set and is required"
        error=1
    fi
fi

if [ ! -v GIT_CONFIG_REPO_URL ] && [ ! -v CONFIG_DIR ];then
    echo "Error: either environment variable GIT_CONFIG_REPO_URL or CONFIG_DIR must be specified"
    error=1
fi
...
ansible-playbook -i ${INV_DIR} playbook-e2e.yml --extra-vars config_dir=${CONF_DIR} --extra-vars iaas_classic_username=${iaas_classic_username} --extra-vars iaas_classic_api_key=${iaas_classic_api_key} --extra-vars ibmcloud_api_key=${IBM_CLOUD_API_KEY} --extra-vars ibm_cp4d_entitlement_key=${ibm_cp4d_entitlement_key} "$@"  
